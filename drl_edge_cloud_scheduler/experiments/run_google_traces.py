...
def run_google_traces(
    config_path: str = "config/config.yaml",
    data_path: str = "data/processed/google_processed.csv",
    ...
    from environment.workload_generator import GoogleClusterLoader
    ...
    print(f"[run_google_traces] Loading dataset: {data_path}")
    loader = GoogleClusterLoader(data_path, config, max_tasks=100_000)
    tasks  = loader.get_all_tasks(max_tasks=10_000)   # use 10K for quick eval
    print(f"[run_google_traces] Loaded {len(tasks):,} tasks")

    # Build env and fit PCA from random sampling
    wl_gen = None
    ...
    if os.path.exists(model_path):
        ...
        print(f"[run_google_traces] Loaded model from {model_path}")
    else:
        print(f"[run_google_traces] No pretrained model found at {model_path}, using random policy")
    ...
    results = evaluator.evaluate_all(
        agents,
        num_episodes=5,
        task_list=tasks[:5000],
        save_path="results/google_traces_results.json",
    )
    print("\n[run_google_traces] Done.")
    return results
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data",   default="data/processed/google_processed.csv")
    parser.add_argument("--model",  default="results/models/proposed_drl_final.pt")
    parser.add_argument("--seed",   type=int, default=42)
    args = parser.parse_args()
    run_google_traces(args.config, args.data, args.model, args.seed)